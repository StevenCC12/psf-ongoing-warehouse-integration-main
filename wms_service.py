...
def get_ghl_order_details(contact_id: str, retries: int = 3, delay_seconds: int = 20) -> dict | None:
    """Fetches order details from GHL using a contact ID, with retries."""
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if transactions:
                transaction_id = transactions[0].get('_id')
                order_id = transactions[0].get('entityId')
                if order_id:
                    print(f"INFO: GHL Step 1/2 - Success: Found transaction {transaction_id}, linked to Order ID: {order_id}")
                    break
    ...
    try:
        ...
        print(f"ERROR during GHL order lookup for order {order_id}: {e}")
        return None

def map_ghl_order_to_wms_payload(ghl_order_data: dict) -> Optional[OngoingWMSOrderPayload]:
    """Maps GHL order data to the OngoingWMSOrderPayload Pydantic model."""
    ...
    contact_id = contact_snapshot.get("id") # Get the contact ID for the customer number
    ...
    for index, item in enumerate(items):
        ...
        if not sku:
            print(f"WARNING: Line item '{item.get('name')}' is missing an SKU. Skipping.")
            continue
...
def create_ongoing_order(wms_payload_model: OngoingWMSOrderPayload) -> bool:
    """Sends the final order payload to Ongoing WMS."""
